...
export const getEmployees = async () => {
    ...
    return snapshot.docs.map((docSnap, i) => {
        ...
        return {
            ...
            idNumber: i + 1,
            ...data,
            monthsWorked: calculateMonthsWorked(data.dateHired, data.dateEnded),
        };
    });
};
...
export const addEmployee = async (employee) => {
    const payload = {
        ...employee,
        monthsWorked: calculateMonthsWorked(employee.dateHired, employee.dateEnded),
        year: new Date(employee.dateHired).getFullYear().toString(),
    };

    if (!employee.gipId || employee.gipId.trim() === "") {
        payload.gipId = await generateNextGipId(employee.name, employee.dateHired);
    }
...
const mapExcelToDatabaseFields = (excelRow) => {
  const fieldMap = {
    ...
    'Previous Company 1': 'workCompany1',
    'Previous Position 1': 'workPosition1',
    'Work Period 1': 'workPeriod1',
    'Previous Company 2': 'workCompany2',
    'Previous Position 2': 'workPosition2',
    'Work Period 2': 'workPeriod2',
 ...
 * Skips duplicates (same name + dateHired).
...
export const uploadEmployeesFromExcel = async (file, onSuccess) => {
    ...
    for (const row of data) {
        const isDuplicate = existing.some(
            (e) => e.name?.toLowerCase().trim() === row.name?.toLowerCase().trim() && e.dateHired === row.dateHired,
        );

        // If duplicate, ask user if they want to accept it
        if (isDuplicate) {
            // eslint-disable-next-line no-restricted-globals
            const acceptDuplicate = confirm(`Duplicate found\nDo you want to accept this duplicate?`);
        ...
        const gipId =
            row.gipId && row.gipId.trim() !== "" ? row.gipId : await generateNextGipId(row.name, row.dateHired);

        const payload = {
            ...row,
            gipId,
            monthsWorked: calculateMonthsWorked(row.dateHired, row.dateEnded),
            year: new Date(row.dateHired).getFullYear().toString(),
        };
