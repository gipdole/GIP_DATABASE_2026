...
export const calculateMonthsWorked = (start, end) => {
    ...
    const dateHired = new Date(start);
    const dateEnded = new Date(end);
    if (!isValid(dateHired) || !isValid(dateEnded)) return 0;

    let months = differenceInMonths(dateEnded, dateHired);
...
export const calculateMonthsAndDaysWorked = (start, end) => {
    ...
    const dateHired = new Date(start);
    const dateEnded = new Date(end);
    if (!isValid(dateHired) || !isValid(dateEnded)) return { months: 0, days: 0 };

    // Calculate full months difference
    let months = differenceInMonths(dateEnded, dateHired);

    // Temporary date to calculate leftover days
    let tempDate = new Date(dateHired);
    tempDate.setMonth(tempDate.getMonth() + months);

    let days = differenceInDays(dateEnded, tempDate);

    // If leftover days negative (dateEnded is before tempDate), adjust
    if (days < 0) {
        months -= 1;
        tempDate.setMonth(tempDate.getMonth() - 1);
        days = differenceInDays(dateEnded, tempDate);
    }
