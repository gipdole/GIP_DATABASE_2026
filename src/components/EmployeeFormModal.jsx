...
const BENEFECIARY_RELATIONSHIP_OPTIONS = ["Father", "Mother", "Spouse", "Daughter", "Son", "Sibling", "Grandparent"];

const EMPLOYMENT_STATUS_OPTIONS = ["NA", "Contract Completed", "Resigned"];
...
const EmployeeFormModal = ({ open, onClose, mode, employee = null, refresh }) => {
    ...
    useEffect(() => {
        if (isEditMode && employee) {
            setForm({
                ...
                monthsWorked:
                    employee.dateHired && employee.dateEnded
                        ? String(calculateMonthsWorked(employee.dateHired, employee.dateEnded))
                        : "",
            });
        } else {
            setForm(defaultForm);
    ...
    // Update monthsWorked when start or Date Endeds change
    useEffect(() => {
        if (form.dateHired && form.dateEnded) {
            const months = calculateMonthsWorked(form.dateHired, form.dateEnded);
            setForm((prev) => ({ ...prev, monthsWorked: String(months) }));
        }
    }, [form.dateHired, form.dateEnded]);
    ...
    const handleSubmit = async () => {
        ...
        if (!gipIdToUse) {
            try {
                gipIdToUse = await generateNextGipId(trimmedName, form.dateHired);
            } catch (err) {
                console.error("GIP ID generation failed:", err);
                alert("Failed to generate GIP ID. Please check Date Hired.");
                return;
            }
        }
        ...
        const payload = {
            ...
            monthsWorked: calculateMonthsWorked(form.dateHired, form.dateEnded),
            year: new Date(form.dateHired).getFullYear().toString(),
        };
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
                ...
                <Stack spacing={2}>
                                ...
                                <Grid item xs={12} size={8}>
                                    <Stack spacing={2}>
                                        <TextField
                                            ...
                                            value={form.name}
                                            onChange={handleChange}
                                            placeholder="Last Name, First Name Middle Name Suffix"
                                            required
                                        />
                                    </Stack>
                                </Grid>
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={12} size={3}>
                                    <Stack spacing={2}>
                                        ...
                                        <TextField
                                            fullWidth
                                            label="Junior High School Degree / Track"
                                            name="secondaryDegree"
                                            value={form.secondaryDegree}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Senior High School Degree / Track"
                                            name="seniorHighDegree"
                                            value={form.seniorHighDegree}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="College Degree / Course"
                                            name="collegeDegree"
                                ...
                                <Grid item xs={12} size={3}>
                                    <Stack spacing={2}>
                                        <TextField
                                            fullWidth
                                            label="Year From"
                                            name="primaryYearFrom"
                                            value={form.primaryYearFrom}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Year From"
                                            name="secondaryYearFrom"
                                            value={form.secondaryYearFrom}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Year From"
                                            name="seniorHighYearFrom"
                                            value={form.seniorHighYearFrom}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Year From"
                                            name="collegeYearFrom"
                                            value={form.collegeYearFrom}
                                            onChange={handleChange}
                                        />
                                    </Stack>
                                </Grid>
                                <Grid item xs={12} size={3}>
                                    <Stack spacing={2}>
                                        <TextField
                                            ...
                                            label="Year To"
                                            name="primaryYearTo"
                                            value={form.primaryYearTo}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Year To"
                                            name="secondaryYearTo"
                                            value={form.secondaryYearTo}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                                            label="Year To"
                                            name="seniorHighYearTo"
                                            value={form.seniorHighYearTo}
                                            onChange={handleChange}
                                        />
                                        <TextField
                                            fullWidth
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} size={4}>
                                <Stack spacing={2}>
                                    <TextField
                                        fullWidth
                                        label="Period of Engagement"
                                        name="workPeriod1"
                                        value={form.workPeriod1}
                                        onChange={handleChange}
                                    />
                                    <TextField
                                        fullWidth
                            ...
                            <Grid item xs={12} size={6}>
                                <Stack spacing={2}>
                                    ...
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                checked={form.victimOfArmedConflict}
                                                onChange={handleCheckboxChange("victimOfArmedConflict")}
                                            />
                                        }
                                        ...
                                        <FormGroup>
                                            ...
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        checked={form.fourP}
                                                        onChange={handleCheckboxChange("fourP")}
                                                    />
                                                }
                            ...
                            <Grid item xs={12} size={6}>
                                <Stack spacing={2}>
                                    ...
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                checked={form.transcriptOfRecords}
                                                onChange={handleCheckboxChange("transcriptOfRecords")}
                                            />
                                        }
                                    ...
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                checked={form.form137138}
                                                onChange={handleCheckboxChange("form137138")}
                                            />
                                        }
                                        label="Form 137/138"
                                    />
                                    ...
                                    <FormControl>
                                        <FormGroup>
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        checked={form.applicationLetter}
                                                        onChange={handleCheckboxChange("applicationLetter")}
                                                    />
                                                }
                                            ...
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        checked={form.barangayCertificate}
                                                        onChange={handleCheckboxChange("barangayCertificate")}
                                                    />
                                                }
                                            ...
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        checked={form.certificationFromSchool}
                                                        onChange={handleCheckboxChange("certificationFromSchool")}
                                                    />
                                                }
                                                label="Certification from School"
                                            />
                                            <TextField
                                                fullWidth
                                                label="Others, specify:"
                    ...
                    <SectionBox title="Government Info">
                        <Stack spacing={2}>
                            ...
                            <TextField
                                ...
                                name="validIdIssued"
                                value={form.validIdIssued}
                                onChange={handleChange}
                                placeholder="MMM/DDD/YYYY or Date Issued"
                            />
                            <TextField
                                fullWidth
                                label="LBP Account Number"
                                name="lbpAccount"
                                value={form.lbpAccount}
                                onChange={handleChange}
                            />
                        </Stack>
                    </SectionBox>
                    ...
                    <SectionBox title="GIP Assignment">
                        <Stack spacing={2}>
                            ...
                            <TextField
                                fullWidth
                                label="Previous Experience (days)"
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} size={4}>
                                <Stack spacing={2}>
                                    <TextField
                                        fullWidth
                                        label="GSIS Beneficiary Relationship"
                                        name="gsisRelationship"
                                        value={form.gsisRelationship}
                                        onChange={handleChange}
                                        select
                                    >
            ...
            <DialogActions>
                <Button onClick={onClose} sx={{ color: "#55C386" }}>
                    Cancel
                </Button>
                <Button
